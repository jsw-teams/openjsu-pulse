...
1. 新建站点目录并安装 Pagekiln：`mkdir my-site && cd my-site && npm i pagekiln`。
2. 用已安装的 CLI 初始化站点：`npx pagekiln init .`。只有在调试框架仓库本身时，才需要用 `node src/bin/pagekiln.mjs init my-site`。
3. 改 `config.yml`：站点名、描述、作者、语言、导航、页脚、robots、llms、feed、插件开关和 discovery 信息都放这里。
4. 改 `content/posts/`：新增或编辑文章。
5. 改 `content/pages/`：调整首页、关于页、归档页、分类页、标签页、搜索页或其他普通页面。
6. 改 `content/assets/`：可替换 `icon-source.png` 或 `og-default-source.png` 后，生成客制化网站图标和网站OG分享图
7. 运行 `pagekiln s` 本地实时预览。
8. 发布前运行 `pagekiln g` 和 `pagekiln c`。
...
- `pagekiln g` / `pagekiln generate` 类似 `hexo generate` / `hexo g`。
- `pagekiln s` / `pagekiln server` 类似 `hexo server` / `hexo s`。
- `pagekiln c` / `pagekiln check` 检查 `dist/`、主题资源、sitemap、feed、Agent discovery 和 WebMCP bootstrap。

`pagekiln init` 会在新项目的 `package.json` 中写入 `generate`、`server`、`check` 等兼容脚本；公开文档优先写 `pagekiln g/s/c`，完整命令 `pagekiln generate/server/check` 也可用。若你在框架仓库或调试环境中直接调用 CLI，也可以使用 `node src/bin/pagekiln.mjs init/generate/server/check`。
...
- `src/bin/pagekiln.mjs` 是 CLI 入口，负责 `init`、`g/generate`、`s/server`、`c/check` 等命令分发，也定义 `pagekiln init` 写入新站点的默认 `package.json` 脚本。
- `themes/<name>/scripts/*.js` 是浏览器端主题脚本。它们只负责页面行为和 consent-aware 功能加载，例如搜索、媒体增强、lightbox、评论加载和 WebMCP 客户端辅助逻辑。
- 修改站点视觉、交互或第三方功能时，优先改 `themes/<name>/theme.yml`、CSS、模板和主题 `scripts/*.js`。只有新增构建期能力、输出文件或通用主题 API 时，才改 `src/`。
...
- 替换 `icon-source.png` 或 `og-default-source.png` 后，直接运行 `pagekiln g`。构建器会从源图生成 ICO、PWA icon 和 OG 分享图并写入最终输出，让站点图标与链接分享资源跟随项目定制化。
- 派生文件可以继续保存在 `content/assets/` 作为模板资源，但它们不是构建入口；`pagekiln g` 优先读取源图并生成输出。框架开发者若需要手动刷新派生文件，可运行 `node src/scripts/generate-neutral-assets.mjs`。
- 图标、PWA 颜色、OG 默认图这类站点身份信息属于 `config.yml` 和 `content/assets/`。主题如果需要自带界面图片，可以自行建立 `themes/<name>/source-assets/`，默认主题不要求这个目录存在。
...
- 构建器未来可以基于这些公开契约生成 `/assets/backend-manifest.json`、OpenAPI 片段和 agent discovery 说明，但 manifest 只能包含公开信息。

后端 API 建议按语义声明缓存和暴露策略，而不是按路径命名硬编码：

- 公开、匿名、幂等读取接口可以允许 CDN 缓存，并按数据新鲜度设置 `max-age` / `stale-while-revalidate`。
- 与用户身份、会话、订阅、私有数据相关的读取接口应使用 `private` 或 `no-store`，避免被共享缓存保存。
- 写入、表单提交、评论发布、webhook、登录、登出、令牌刷新等接口应默认 `no-store`，并配合输入校验、Origin/CORS 策略、请求大小限制、rate limit 和 CAPTCHA/Turnstile 等防滥用措施。
- 管理接口、高权限接口和内部工具接口不应进入 public manifest、OpenAPI、API catalog、MCP server card 或前端 discovery。

这样可以保留静态站的访问性能和 CDN 命中率，同时给评论、表单、搜索增强、webhook、AI 工具、用户态功能和其他后端能力留下清晰扩展入口。
